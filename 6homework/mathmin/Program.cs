...
namespace ConsoleApp2
{
    ...
    class Program
    {
        static int GetInt(int max)
        {
            ...
            Console.Write("Неверный ввод. Пожалуйста повторите: ", max);
                else return x;
        }
        ...
        static double scndD(double x)
        {
            return x * x;
        }
        static double thrd(double x)
        {
            return x * x * x;
        }
        ...
        {
            List<Fun> functions = new List<Fun> { new Fun(scndD), new Fun(thrd), new Fun(mySqrt), new Fun(Sin) };
            ...
            Console.WriteLine("Задайте отрезок для нахождения минимума в формате 'х1(через пробел)х2':");
            double start = 0;
            double end = 0;
            GetInterval(out start, out end);
            Console.WriteLine("Задайте величину шага:");
            ...
            Console.WriteLine("Минимальное значение равняется: {0:0.00}", min);
            Console.ReadKey();
        }
    }
}